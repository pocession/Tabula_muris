...
### Get data with the most 200 variant genes

To reduce the computation, we will only choose top 200 genes with the most variance for further analysis.

```{r variance, warnings=FALSE}
## Calculate the variance, sort the gene based on the variance, and get the top 200 variant genes
Gene.Var <- RowVar((mtx.sparse)) %>%
  sort(decreasing = TRUE)
Gene.subset <- Gene.Var[1:1000]
...
for (imouse in mouse.list) {
  ...
  pseudobulk.mouse[[imouse]] <- getPseudobulk(mtx.sparse.subset, celltype)
}
```
...
major_cell_population <- c("intestinal crypt stem cell",
...
```{r differential_analysis, warning=FALSE}
res.list <- list()
for (icell in major_cell_population) {
...
count.list <- list()
for (icell in major_cell_population) {
  count.list[[icell]] <- as.data.frame(counts(dds.list[[icell]], normalized=FALSE))
}
```
...
for (icell in major_cell_population) {
  ...
  df <- df %>%
    ...
    left_join(res.list[[icell]][["18"]][,c("symbol","DEG")],by="symbol") %>%
    left_join(res.list[[icell]][["24"]][,c("symbol","DEG")],by="symbol") %>%
    mutate(DEG.both = ifelse((DEG.x != "None") & (DEG.y != "None"), TRUE, FALSE))
  colnames(df) <- c("month","id","gender","symbol","count","mouse.id","DEG.18","DEG.24","DEG.both")
